...
if $INSTALL_ASCENT; then
    ...
    if $USE_SPACK_CACHE; then
        ...
        spack install -n ascent@0.6.0+openmp+shared~adios~mfem+fortran+test~python+serial+mpi~cuda+vtkh ^vtk-h@0.6.5~cuda~openmp+mpi+serial+shared ^conduit@0.6.0~adios~doc~doxygen+fortran+hdf5+hdf5_compat+mpi~python+shared~silo+test~zfp
    fi
